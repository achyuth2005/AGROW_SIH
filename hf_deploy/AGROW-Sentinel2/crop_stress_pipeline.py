...
class CropStressPipeline:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.config.sh_client_id = os.environ.get('SH_CLIENT_ID', "sh-709c1173-fc33-4a0e-90e4-b84161ed5b9d")
        self.config.sh_client_secret = os.environ.get('SH_CLIENT_SECRET', "IdopxGFFr3NKFJ4Y2ywJRVfmM5eBB9b4")
...
"""
    ...
    def run(self, center_lat: float, center_lon: float, crop_type: str,
            ...
            farmer_context: dict, output_path: str = None, skip_llm: bool = False) -> dict:
        """
        ...
        Args:
            ...
            skip_llm: Skip LLM analysis step to save API calls (for chatbot use)
        ...
        try:
            ...
            # Step 4: Run LLM analysis (SKIP if skip_llm=True)
            if skip_llm:
                logger.info("SKIPPING LLM analysis (chatbot will do its own reasoning)")
                llm_analysis = {"skipped": True, "reason": "Called from chatbot"}
            else:
                try:
                    ...
                    logger.warning(f"LLM analysis failed: {llm_error}. Using fallback response.")
                    # Provide fallback analysis based on vegetation indices
                    # Calculate levels from actual index data
                    indices = summary_report.get('indices', {})
                    
                    def get_level_from_value(value, thresholds=(0.3, 0.6)):
                        if value is None:
                            return "Moderate"
                    ...
                    smi_val = indices.get('SMI', {}).get('latest', {}).get('mean', 0.5)
                    ndvi_val = indices.get('NDVI', {}).get('latest', {}).get('mean', 0.5)
                    
                    llm_analysis = {
                        "soil_moisture": {
                            "level": get_level_from_value(smi_val),
                            "maximum_value": indices.get('SMI', {}).get('max_in_field', 0.7),
                            "minimum_value": indices.get('SMI', {}).get('min_in_field', 0.3),
                            "analysis": "Analysis unavailable - API limit reached"
                        },
                        "soil_salinity": {
                            "level": "Moderate",
                            "analysis": "Analysis unavailable - API limit reached"
                        ...
                        "soil_fertility": {
                            "level": get_level_from_value(ndvi_val),
                            "analysis": "Analysis unavailable - API limit reached"
                        },
                        "pest_risk": {
                            "level": "Moderate",
                        ...
                        "stress_zone": {
                            "level": "Low" if stress_llm_context.get('field_statistics', {}).get('overall_stress', {}).get('mean', 0.5) < 0.3 else "Moderate",
                            "analysis": "Analysis unavailable - API limit reached"
                        },
                        "overall_health": {
                            "status": "fair",
                            "key_concerns": ["LLM analysis unavailable"],
                            "recommendations": ["System is operating in fallback mode due to API limits"]
                        },
                        "overall_soil_health": 0.5,
                        "overall_biorisk": 0.3,
                        "_fallback": True,
                        "_error": str(llm_error)
                    }
