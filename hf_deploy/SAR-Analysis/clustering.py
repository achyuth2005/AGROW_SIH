...
class StressAnalyzer:
    ...
    def cluster_embeddings(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        # Adjust n_clusters if samples are too few
        if len(embeddings) < self.n_clusters:
            print(f"Warning: Number of samples ({len(embeddings)}) is less than n_clusters ({self.n_clusters}). Adjusting n_clusters.")
    ...
    def detect_anomalies(self, embeddings: np.ndarray) -> tuple:
        ...
        if score_range > 0:
            anomaly_scores = (anomaly_scores - anomaly_scores.min()) / score_range
        else:
            anomaly_scores = np.zeros_like(anomaly_scores) # Or ones, depending on interpretation. 0 means most anomalous usually? No, 0 is usually least anomalous in this normalization context if we map min to 0. 
            # Actually, if all scores are same, they are equally anomalous/normal. Let's set to 0.5 or 0.
            # If we want to be safe, let's keep them as is (raw scores) or just 0.
            # Let's set to 0.5 to indicate neutral/uncertain.
            anomaly_scores = np.full_like(anomaly_scores, 0.5)
